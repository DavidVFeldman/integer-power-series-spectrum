...
This file is organized as a sequence of lemmas mirroring the paper's proof structure.
-/
...
theorem exists_enum_of_effectiveDivisor (D : EffectiveDivisor) :
  ...
  refine ⟨a, ?_, ?_, ?_, ?_⟩
  rotate_left 3
  · -- values are genuine support points or the pad `2`
    intro k
    by_cases h : ∃ σ : ZigmaT, ι σ = k
    · exact Or.inl (haS k h).1
    · exact Or.inr (ha_of_not_ex h)
  · -- `a k ≠ 0`
    intro k
    by_cases h : ∃ σ : ZigmaT, ι σ = k
    · exact (haS k h).2
...
theorem exists_Mtest_of_coeffSeq (a c : ℕ → ℂ) (ha0 : ∀ k, a k ≠ 0)
  ...
  · intro k z hz
    ...
    · simp only [hkBad, if_true]
      ...
      have : 0 ≤ 2 * B k := by have := hBnonneg k; linarith
      linarith [hM k z hz]
    · simp only [hkBad, if_false, zero_add]
      ...
      have htail : ρ ^ (k + 2) / (1 - ρ) ≤ (r / s) ^ (k + 2) / (1 - r / s) := by
        ...
        have h5 : (0:ℝ) < 1 - ρ := lt_of_lt_of_le h2 h4
        gcongr
