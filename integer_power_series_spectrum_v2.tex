...
\title{The Prime Spectrum of Rings of\\Integer-Coefficient Power Series on the Disk}
...
We also use, from \cite{PaperI}, that every $f \in \OD$ factors as $f =
g\,u$ with $g \in \mR$ and $u \in \OD^\times$, that consequently the
...
\begin{remark}
\label{rem:padic}
The primes $Q_p$ are $p$-adic analogues of the point-evaluation ideals
$P_a$ of Section~\ref{sec:pointeval}: where $P_a$ is the kernel of
...
The coprimality hypothesis cannot be dropped: for any rational prime
$p$, the pair $f = g = p$ has empty (hence disjoint) zero sets in $\D$,
...
\item \textbf{Classification of type~(iii) maximal ideals.}  The
  ...
  ideals.  Whether every type~(iii) maximal ideal is of this
  form---equivalently, whether $\mathfrak{M} \cap \mathfrak{P}_1$
  consists exactly of the $P_a$---is the central open question.  A
